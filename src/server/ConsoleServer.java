    ...
    public void sendPrivateMsg(ClientHandler nickFrom, String nickTo, String msg) {
        for (ClientHandler c : users) {
            if (c.getNickname().equals(nickTo)) {
                if (!nickFrom.getNickname().equals(nickTo) & !c.checkBlackList(nickFrom.getNickname())) {
                    // добавление второй части в if не дает писать тому, кто занес в черный список
                    c.sendMsg(nickFrom.getNickname() + ": [Send for " + nickTo + "] " + msg);
                    nickFrom.sendMsg(nickFrom.getNickname() + ": [Send for " + nickTo + "] " + msg);
                } else {
                    // так же отправляется сообщение, о том, что пишешь самому себе или в черном списке
                    nickFrom.sendMsg(nickFrom.getNickname() + ": вы пишите самому себе или занесены в черный список");
                }
            }
        }
    }
